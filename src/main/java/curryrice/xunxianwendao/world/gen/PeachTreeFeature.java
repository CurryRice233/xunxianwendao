...
public class PeachTreeFeature extends AbstractTreeFeature<NoFeatureConfig>{
	private static final IBlockState LOG = BlockList.PEACH_LOG.getDefaultState();
	private static final IBlockState LEAF = BlockList.PEACH_LEAF.getDefaultState();

	public PeachTreeFeature(boolean notify) {
		super(notify);
	}
	...
	public boolean place(Set<BlockPos> changedBlocks, IWorld worldIn, Random rand, BlockPos position) {
	      ...
	      if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getWorld().getHeight()) {
	         ...
	         if (!flag) {
	            ...
	            boolean isSoil = worldIn.getBlockState(position.down()).canSustainPlant(worldIn, position.down(), net.minecraft.util.EnumFacing.UP, (net.minecraft.block.BlockSapling)Blocks.OAK_SAPLING);
