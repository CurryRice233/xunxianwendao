...
import curryrice.xunxianwendao.client.render.RenderXunXianWenDao;
...
public class EventLoader {
    ...
    public void onEntityJoinWorld(EntityJoinWorldEvent event) {
        if (!event.getWorld().isRemote && event.getEntity() instanceof EntityPlayer) {
            event.getEntity().getCapability(CapabilityLoader.xunxianwendaoCapability).ifPresent(c -> {
                c.sync((EntityPlayerMP) event.getEntity());
            });
        }
    }
    ...
    public void onPlayerRespawn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerRespawnEvent event) {
        event.getPlayer().getCapability(CapabilityLoader.xunxianwendaoCapability).ifPresent(c -> {
            c.sync((EntityPlayerMP) event.getPlayer());
        });
    }
    ...
    @OnlyIn(Dist.CLIENT)
    @SubscribeEvent
    public void onGuiIventoryOpen(RenderGameOverlayEvent.Post event) {
        if (!event.isCancelable() && event.getType() == RenderGameOverlayEvent.ElementType.EXPERIENCE) {
			Minecraft.getInstance().player.getCapability(CapabilityLoader.xunxianwendaoCapability).ifPresent(cap -> {
			    if(cap.getLevel()>0){
                    RenderXunXianWenDao.renderMagicBar(cap);
                }
            });
        }
    }
