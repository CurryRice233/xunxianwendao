...
public class BlockThunderWood extends BlockBush implements IElements {
    private static final VoxelShape SHAPE = Block.makeCuboidShape(4D, 0.0D, 4D, 12D, 12D, 12D);
    private static Elements elements = new Elements(0,1,0,0,0);
    public BlockThunderWood() {
        super(Block.Properties.create(Material.PLANTS).needsRandomTick().doesNotBlockMovement().hardnessAndResistance(0.0F).sound(SoundType.WOOD));
        this.setRegistryName(new ResourceLocation(XunXianWenDao.MODID, "thunder_wood"));
    }
    ...
    public void randomTick(IBlockState state, World worldIn, BlockPos pos, Random random) {
        ...
        if(random.nextInt(10)==0)
            worldIn.addWeatherEffect(new EntityLightningBolt(worldIn,pos2.getX(),worldIn.getHeight(Heightmap.Type.MOTION_BLOCKING,pos2.getX(),pos2.getZ()),pos2.getZ(),false));
